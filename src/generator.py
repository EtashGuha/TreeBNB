...
# def generate_capacited_facility_location(rng, filename, n_customers, n_facilities, dn):
#     """
...
#     """
#     ratio = n_customers / n_facilities
#     c_x = rng.randint(0,1001, n_customers) / 1000
#     c_y = rng.randint(0,1001, n_customers) / 1000
#
#     f_x = rng.randint(0,1001, n_facilities) / 1000
#     f_y = rng.randint(0,1001, n_facilities) / 1000
#
#     demands = rng.randint(1, 100+1, size=n_customers)
#     if dn == 5:
#         capacities = (800 + rng.randint(0, 3+1, size=n_facilities) * 200) * ratio
#     elif dn == 10:
#         # capacities = np.ones(n_facilities,) * 500
#         capacities = (500 + rng.randint(0, 3+1, size=n_facilities) * 100) * ratio
#     elif dn == 15:
#         capacities = (500 + rng.randint(0, 3 + 1, size=n_facilities) * 75) * ratio
#     else:
#         raise NotImplementedError
#     # transportation costs
#     trans_costs = np.sqrt(
#         (c_x.reshape((-1, 1)) - f_x.reshape((1, -1))) ** 2 \
#         + (c_y.reshape((-1, 1)) - f_y.reshape((1, -1))) ** 2) * (1+rng.rand(n_customers,n_facilities)/4) * demands.reshape((-1, 1))
#
#     P0 = rng.choice(n_facilities, dn+1, replace=False)
#     D0 = np.sum(np.min(trans_costs[:,P0], axis=1))
#     D1 = np.sum(np.min(trans_costs[:,P0[:dn-1]], axis=1))
#     fixed_costs = (D1-D0) * (0.75 + rng.rand(n_facilities)*0.5)
#     # fixed_costs = rng.randint(100, 110+1, size=n_facilities) * np.sqrt(capacities) \
#     #         + rng.randint(10+1, size=n_facilities)
...
def generate_capacited_facility_location(random, filename, n_customers, n_facilities, ratio):
    ...
    trans_costs = (np.sqrt(
            (c_x.reshape((-1, 1)) - f_x.reshape((1, -1))) ** 2 \
            + (c_y.reshape((-1, 1)) - f_y.reshape((1, -1))) ** 2) * 10).astype(int) * demands.reshape((-1, 1))
...
if __name__ == '__main__':
    ...
    if args.problem == 'setcover':
        ...
        #     lp_dir = f'../data/instances/setcover/valid_{nrows}r_{ncols}c_{dens}d_{max_coef}mc_{args.seed}se/instance_{i+1}'
        #     os.makedirs(lp_dir)
        #     lp_dirs.append(lp_dir)
        ...
        for filename, ncs, nfs, r in zip(filenames, ncustomerss, nfacilitiess, ratios):
            print(f"  generating file {filename} ...")
            generate_capacited_facility_location(rng, filename, n_customers=ncs, n_facilities=nfs, ratio=ratio)
